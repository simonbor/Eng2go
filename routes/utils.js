exports.win1255ToUnicode = function (source, undefHandler /* function! */) {
    // http://expertland.net/question/u3238b821a53z4sl48qae1qr1r3a6z3c0/detail.html
...
var win1255Encoding = {
    ...
    l = source.length;
    while (l--) {
        var char = String.fromCharCode(source[i]);
        if (win1255Encoding[char])
            dest += win1255Encoding[char];
        else {
            if (undefHandler) {
                dest += undefHandler(char);
            } else {
                dest += '\uFFFD';
            }
        }
